...
function randomArrayValue(rng: () => number, depth: number): PhpValue {
	...
	for (let i = 0; i < length; i++) {
		entries.push({
			key:
				rng() > 0.4
					? ({ type: 'int', value: i } as const)
					: ({ type: 'string', value: `k${i}` } as const),
...
function randomObjectValue(rng: () => number, depth: number): PhpValue {
	...
	for (let i = 0; i < length; i++) {
		...
		const name = `p${i}`;
		const propClassName =
			visibility === 'private' && rng() > 0.5 ? `Other${Math.floor(rng() * 3)}` : undefined;
...
function isSamePhpValue(a: PhpValue, b: PhpValue): boolean {
	...
	switch (a.type) {
		...
		case 'enum':
			return (
				a.className === (b as typeof a).className &&
				a.caseName === (b as typeof a).caseName
			);
		case 'reference':
			return (
				a.index === (b as typeof a).index &&
				a.isObject === (b as typeof a).isObject
			);
	}
}
