...
test('toggle groups and collapsible expose state semantics', async ({ page }) => {
	...
	await expand.click();
	await expect(page.getByRole('button', { name: 'Collapse stats' })).toHaveAttribute(
		'aria-expanded',
		'true',
	);
});
...
test('monaco runtime does not request external CDN assets', async ({ page }) => {
	...
	const disallowedRequests = requests.filter((url) =>
		disallowedDomains.some((domain) => url.includes(domain)),
	);
